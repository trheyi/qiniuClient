<template>
  <div class="layout-header">
    <div class="full">
      <Select class="input no-drag" v-model="bucket.domain" v-if="bucket.name && bucket.domains && bucket.domains.length > 0" @on-change="onChangeDomain">
        <Option v-for="item of bucket.domains" :value="item" :key="item">{{ item }}</Option>
      </Select>
      <!-- TODO:添加自定义域名功能-->
      <Input
        class="input no-drag"
        v-model="bucket.domain"
        v-if="bucket.domains.length === 0"
    ...
    <Input class="input-search no-drag" v-model="search" placeholder="搜索" :icon="search.length > 0 ? 'md-close-circle' : ''" @on-enter="actionBtn(2)" @on-click="clearSearch" v-show="bucket.name" />

    <!-- 同步对话框-->
    <Modal v-model="model_merge.show" title="请选择同步方式" class-name="vertical-center-modal" @on-ok="syncFolder">
      <template>
...
export default {
  ...
  methods: {
    ...mapActions([types.setup.a_customedomain]),
    onChangeDomain(e) {
