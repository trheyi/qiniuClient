<template>
  <div ref="content" class="layout-content">
    <v-contextmenu ref="folderMenu" @contextmenu="handleFolderMenu">
      ...
      <v-contextmenu-item v-feature:refreshCDN="bucket.key" @click="handleFolderMenuClick(5)">刷新CDN</v-contextmenu-item>
      <v-contextmenu-item v-feature:refreshCDN="bucket.key" divider></v-contextmenu-item>
      ...
      <v-contextmenu-item divider></v-contextmenu-item>
      <v-contextmenu-item @click="handleFolderMenuClick(0)"><span style="color: red; width: 300px;">删除</span></v-contextmenu-item>
    </v-contextmenu>
    <v-contextmenu ref="fileMenu" @contextmenu="handleFileMenu">
      ...
      <v-contextmenu-item divider></v-contextmenu-item>
      <v-contextmenu-item v-feature:refreshCDN="bucket.key" @click="handleFileMenuClick(8)">刷新CDN</v-contextmenu-item>
      <v-contextmenu-item v-feature:refreshCDN="bucket.key" divider></v-contextmenu-item>
      ...
      <v-contextmenu-item divider></v-contextmenu-item>
      <v-contextmenu-item @click="handleFileMenuClick(0)"><span style="color: red; width: 300px;">删除</span></v-contextmenu-item>
    </v-contextmenu>
    <virtual-list :size="123" :remain="remain1" :bench="10" :debounce="500" class="grid2" v-if="type === 1" key="1">
      <div v-for="(items, index1) of getFileByGrid(files)" class="grid2-item" :key="index1">
        <Card v-for="(file, index) of items" :key="file.key" class="card" :padding="0" :bordered="false" v-bind:class="{ 'item-select': selection.indexOf(files.indexOf(file)) !== -1 }">
          <div class="item" @click="clickItem(file, files.indexOf(file))" v-contextmenu="file._directory ? 'folderMenu' : 'fileMenu'" :index="file">
            ...
            <template v-else>
              ...
              <div class="btn">
                <Button shape="circle" size="small" icon="md-download" @click.stop="fileAction(file, 0)" style="background: #ffffff;"></Button>
                <Button shape="circle" size="small" icon="md-clipboard" @click.stop="copyFileUrl(file)" style="background: #ffffff;"></Button>
                <Button type="error" shape="circle" size="small" icon="md-trash" @click.stop="resourceRemove(file)"></Button>
              </div>
            </template>
          </div>
        </Card>
    ...
    <viewer :options="options" :images="previewImages" ref="viewer" @inited="inited" style="display: none;">
      <img class="image-wrapper" v-for="src in previewImages" :key="src" :src="src" :data-source="src" :alt="src.split('?image=').pop()" />
    </viewer>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getImgUrlWithStyle(file) {
      ...
      switch (this.$storage.key) {
        ...
        //     if (parseInt(this.bucket.permission) === 1) {
        //         //如果是私密空间直接显示原图
        //         imageSrc = url
        //     } else {
